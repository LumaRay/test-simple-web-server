...
const TEST_CODE: &str = r#"jel({"h1": "test"})"#;
...
//     format!("<html><head></head><body><script type='text/javascript'>{JEL_CODE}</script></body></html>")
// }
...
async fn index() -> HttpResponse {
    HttpResponse::Ok()
        ...
        .body(format!("<html><head></head><body><script type='text/javascript'>{JEL_CODE}{TEST_CODE}</script></body></html>"))
}
...
async fn main() -> std::io::Result<()> {
    HttpServer::new(|| {
        App::new()
            // .route("/hello", web::get().to(|| async { "<b>Hello World!</b>" }))
            .service(index)
    })
    .bind(("127.0.0.1", 8080))?
    .run()
