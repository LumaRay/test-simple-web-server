...
// https://faun.pub/kubernetes-and-minikube-setup-on-linux-mint-fb746e768c22
// https://techviewleo.com/how-to-install-virtualbox-on-linux-mint/
// https://jongsma.wordpress.com/2021/11/29/docker-and-the-virtualbox-host-only-networks/
// https://stackoverflow.com/questions/54034445/unable-to-start-minikube-stuck-on-starting-cluster-components
...
// 	Chi       []Node `json:"chi,omitempty"`
// 	InnerHTML string `json:"innerHTML,omitempty"`
// }

// type Node map[string]Attr
...
func getRoot(w http.ResponseWriter, r *http.Request) {
	...
	io.WriteString(w, fmt.Sprintf(`<html><head></head><body><script type="text/javascript">%s</script></body></html>`, JEL_CODE))
	//node := &H1{H1: "Frank"}
	...
	// 	return
	// }
	// io.WriteString(w, fmt.Sprintf(`<script type="text/javascript">jel(%s)</script>`, string(b)))
	io.WriteString(w, fmt.Sprintf(`<script type="text/javascript">jel({"h1": "test"})</script>`))
}
